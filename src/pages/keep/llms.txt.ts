...
import { SITE_URL, MAIN_CATEGORIES, OPTIONAL_CATEGORIES, INTRO } from '../../lib/keep-llms';
...
export const GET: APIRoute = async () => {
  ...
  lines.push('# Keep', '');
  lines.push('> Policy engine for AI agent tool calls.', '');
  ...
  for (const category of MAIN_CATEGORIES) {
    ...
    for (const doc of categoryDocs) {
      ...
      const url = `${SITE_URL}/keep/${category}/${slug}.md`;
  ...
  lines.push('');
  lines.push('---', '');
  lines.push(`> Full content: [llms-full.txt](${SITE_URL}/keep/llms-full.txt)`);
  lines.push('');
