...
import { SITE_URL, MAIN_CATEGORIES, OPTIONAL_CATEGORIES, INTRO } from '../../lib/gatekeeper-llms';
...
export const GET: APIRoute = async () => {
  ...
  lines.push('> Credential-injecting TLS-intercepting proxy.', '');
  ...
  for (const category of MAIN_CATEGORIES) {
    ...
    for (const doc of categoryDocs) {
      ...
      const url = `${SITE_URL}/gatekeeper/${category}/${slug}.md`;
  ...
  lines.push('');
  lines.push('---', '');
  lines.push(`> Full content: [llms-full.txt](${SITE_URL}/gatekeeper/llms-full.txt)`);
  lines.push('');
